...
export function Header() {
  ...
  const logoSrc = company?.logo || LOGOS.logo;
  const companyName = company?.companyName || "Kollimalai Arasan";
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          <Link href="/" className="flex items-center gap-2 sm:gap-3 group">
            <Image
              ...
              height={80}
              priority
              className="w-[45px] h-[45px] sm:w-[55px] sm:h-[55px] md:w-[65px] md:h-[65px] object-contain group-hover:scale-105 transition-transform duration-300 active:scale-95 rounded-lg"
              unoptimized={Boolean(company?.logo)}
            />
            <span className="font-hanken font-bold text-lg sm:text-xl md:text-2xl text-primary-700 tracking-tight transition-colors">
              {companyName}
            </span>
          </Link>
        </div>
